...
@pytest.mark.anyio
async def test_other_bots_are_saved_as_context() -> None:
    cog, send, bot = _cog()
    message = _message(content="", bot=True)
...
@pytest.mark.anyio
async def test_external_webhooks_trigger_responses_with_webhook_identity() -> None:
    cog, send, bot = _cog()
    message = _message(content="", bot=True, webhook_id=777)
    embed = discord.Embed(title="アンケート結果", description="参加者10人")
    embed.add_field(name="賛成", value="8")
    embed.add_field(name="反対", value="2")
    message.embeds = [embed]
    ...
    assert send.await_args is not None
    save_command = send.await_args.args[0]
    ...
    assert save_command.author_name == "Poll Bot"
    assert cog._queue.qsize() == 1
    queued = cog._queue.get_nowait()
    assert "賛成: 8" in queued.command.content
    assert "反対: 2" in queued.command.content
    bot.get_context.assert_not_awaited()


@pytest.mark.anyio
async def test_external_webhooks_fall_back_to_main_text_channel() -> None:
    cog, send, bot = _cog()
    message = _message(channel_id=789, content="External alert", webhook_id=777)

    await cog.on_message(message)

    save_command = send.await_args_list[0].args[0]
    assert isinstance(save_command, SaveDiscordChatCommand)
    assert save_command.channel_id == SCOPE.channel_id
    assert save_command.author_kind is AuthorKind.WEBHOOK
    assert cog._queue.qsize() == 1
    queued = cog._queue.get_nowait()
    assert queued.command.channel_id == SCOPE.channel_id
    assert queued.command.delivery_channel_id == SCOPE.channel_id
    bot.get_context.assert_not_awaited()


@pytest.mark.anyio
async def test_external_webhooks_from_another_guild_are_saved_only() -> None:
    cog, send, bot = _cog()
    message = _message(guild_id=999, channel_id=789, webhook_id=777)
    ...
    save_command = send.await_args.args[0]
    assert isinstance(save_command, SaveDiscordChatCommand)
    assert save_command.guild_id == "999"
    assert save_command.channel_id == "789"
    assert cog._queue.empty()
    bot.get_context.assert_not_awaited()
