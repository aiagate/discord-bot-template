...
async def test_generates_with_current_author_and_publishes_correct_identity(
    master_id: str | None,
) -> None:
    source = _source()
    handler, generator, publisher, history, memory, uow, command = _handler(
        source, master=DiscordMaster(master_id)
    )
    ...
    assert history.get_recent_history.await_args.kwargs["before"][0].tzinfo is UTC
    assert memory.get_relevant.await_args.kwargs["before"][0].tzinfo is UTC
    assert memory.get_selection_summaries.await_args.kwargs["before"][0].tzinfo is UTC
    ...
    assert payload["master"] == expected_master
    selection = json.loads(generator.select_character.await_args.kwargs["user_content"])
    assert selection["master"] == expected_master
    assert selection["current"]["author_id"] == "alice"
    assert selection["current"]["occurred_at"] == "2026-09-12 10:00:00 JST"
    ...
    assert payload["current"]["is_master"] is (False if master_id is not None else None)
    assert payload["current"]["message_id"] == source.external_message_id
    assert payload["current"]["occurred_at"] == "2026-09-12 10:00:00 JST"
    assert payload["history"] == []
    speech = publisher.publish.await_args.args[0]
    assert speech.username == "Dorothy【メイド長】"
...
@pytest.mark.anyio
@pytest.mark.parametrize("origin", ["conversation", "times"])
async def test_selected_character_receives_only_its_character_memory(
    origin: str,
) -> None:
    source = _source()
    ...
    memory_item = CharacterMemory(
        character_id="Dorothy",
        source_message_id="times:80:00" if origin == "times" else "memory-source",
        sequence=0,
        content="ユーザーは朝に作業する。",
    ...
    assert payload["character_memory"] == [
        {
            "content": "ユーザーは朝に作業する。",
            "observed_at": "2026-09-12 10:00:00 JST",
            "origin": origin,
        }
    ]
    assert memory_item.observed_at.tzinfo is UTC
...
async def test_character_selection_receives_each_character_working_summary() -> None:
    ...
    assert by_name["Dorothy"]["memory_summary_observed_at"] == "2026-09-12 10:59:00 JST"
    assert summary.observed_at.tzinfo is UTC
    assert by_name["Eris"]["memory_summary"] == ""
    assert by_name["Eris"]["memory_summary_observed_at"] is None
...
async def test_external_bot_results_are_separate_from_current_user_content() -> None:
    ...
    assert payload["history"][0]["occurred_at"] == "2026-09-12 09:59:00 JST"
    selection = json.loads(generator.select_character.await_args.kwargs["user_content"])
    assert selection["history"][0]["occurred_at"] == "2026-09-12 09:59:00 JST"
    assert "賛成: 8" in payload["history"][0]["content"]
    assert payload["current"]["author_id"] == "alice"


@pytest.mark.anyio
async def test_external_webhook_identity_reaches_response_prompt() -> None:
    master_id = "672491948375932937"
    handler, generator, _, history, _, _, command = _handler(
    ...
    report = ChatMessage.create_discord(
        guild_id=SCOPE.guild_id,
        channel_id=SCOPE.channel_id,
        external_sender_id="777",
        author_kind=AuthorKind.WEBHOOK,
        author_name="Alert Feed",
        content=MessageContent.text("重要な外部通知"),
        occurred_at=datetime(2026, 9, 12, 0, 59, tzinfo=UTC),
    )
    history.get_recent_history.return_value = Ok([report])
    ...
    payload = json.loads(generator.generate.await_args.kwargs["user_content"])
    webhook = payload["history"][0]
    assert webhook["author_id"] == "777"
    assert webhook["author_name"] == "Alert Feed"
    assert webhook["author_kind"] == "webhook"
    assert webhook["is_master"] is False
    assert webhook["content"] == "重要な外部通知"


@pytest.mark.anyio
async def test_configured_master_author_is_marked_in_the_prompt() -> None:
    master_id = "672491948375932937"
    source = _source(author=master_id)
    ...
    payload = json.loads(generator.generate.await_args.kwargs["user_content"])
    selection = json.loads(generator.select_character.await_args.kwargs["user_content"])
    assert payload["current"]["is_master"] is True
    assert selection["current"]["is_master"] is True


@pytest.mark.anyio
async def test_master_context_reaches_selection_and_response_prompts() -> None:
    context = "マスターは結論を先に、短く答えてほしい。"
    handler, generator, _, _, _, _, command = _handler(
        master=DiscordMaster("672491948375932937", context)
    )
    ...
    payload = json.loads(generator.generate.await_args.kwargs["user_content"])
    selection = json.loads(generator.select_character.await_args.kwargs["user_content"])
    assert payload["master"]["context"] == context
    assert selection["master"]["context"] == context
    assert (
