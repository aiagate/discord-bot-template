...
from app.domain.value_objects import AuthorKind, MessageContent
...
def test_prompt_message_marks_only_the_configured_user_as_master() -> None:
    master = DiscordMaster("672491948375932937")
    user = ChatMessage.create_discord(
        guild_id="456",
        channel_id="123",
        external_sender_id="672491948375932937",
        author_kind=AuthorKind.USER,
        content=MessageContent.text("マスターの投稿"),
        occurred_at=datetime(2026, 9, 12, 7, 0, tzinfo=UTC),
    )
    webhook = ChatMessage.create_discord(
        guild_id="456",
        channel_id="123",
        external_sender_id="672491948375932937",
        author_kind=AuthorKind.WEBHOOK,
        content=MessageContent.text("外部Webhookの投稿"),
        occurred_at=datetime(2026, 9, 12, 7, 1, tzinfo=UTC),
    )

    assert prompt_message(user, master=master)["is_master"] is True
    assert prompt_message(webhook, master=master)["is_master"] is False
    assert prompt_message(user)["is_master"] is None


def test_master_context_is_trimmed_and_included_without_an_id() -> None:
    master = DiscordMaster(context="  朝は短く答えてください。\n")

    assert master.context == "朝は短く答えてください。"
    assert master.to_prompt() == {"context": "朝は短く答えてください。"}


@pytest.mark.parametrize("context", ["", "  ", "x" * (MAX_MASTER_CONTEXT_LENGTH + 1)])
def test_master_context_rejects_empty_or_overlong_values(context: str) -> None:
    with pytest.raises(ValueError):
        DiscordMaster(context=context)
