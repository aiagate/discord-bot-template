...
async def test_name_avatar_actual_attachment_and_receipt(
    ...
    assert options["username"] == character.display_name
    ...
    assert result.content == text and result.username == character.display_name
...
@pytest.mark.anyio
async def test_empty_allowed_channels_use_webhook_destinations(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    reporter, _, _, _ = _reporter(monkeypatch)

    assert await reporter.initialize(frozenset()) == frozenset({"123"})
