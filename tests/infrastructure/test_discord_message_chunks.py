...
@pytest.mark.parametrize("prefix", ["あ" * 50, "😀" * 25])
@pytest.mark.parametrize("mention", ["<@672491948375932937>", "<@!672491948375932937>"])
def test_user_mention_at_chunk_boundary_stays_intact(prefix: str, mention: str) -> None:
    content = prefix + mention + "い" * 30
    parts = split_discord_content(content, limit=64)

    assert "".join(parts) == content
    assert sum(part.count(mention) for part in parts) == 1
    assert all(len(part.encode("utf-16-le")) // 2 <= 64 for part in parts)
