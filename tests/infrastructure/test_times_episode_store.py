...
async def test_times_episode_store_lifecycle_and_progress(
    ...
    posts = (
        TimesPost(character_name="Dorothy", content="Post 1"),
...
async def test_times_episode_store_pending_and_completed_queries(
    ...
    pending_res = await store.pending()
    ...
    completed_res = await store.get_recent_completed(limit=5)
    ...
    filtered_res = await store.get_recent_completed(
        limit=5, before=base_time + timedelta(minutes=2)
    )
