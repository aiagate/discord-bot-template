...
async def test_generate_times_episode_success() -> None:
            ...
            "posts": [
                {
                    "character_name": "Dorothy",
                    "content": "お帰りなさいませ。",
                    "memory_candidates": [" Timesで時刻の基準を話した。 ", " "],
                    "selection_summary": " 時刻の扱いを仲間と確認した。 ",
                },
    ...
    assert res.value[0].memory_candidates == ("Timesで時刻の基準を話した。",)
    assert res.value[0].selection_summary == "時刻の扱いを仲間と確認した。"
    assert res.value[1].character_name == "Eris"
    assert res.value[1].content == "待っていたぞ。"
    assert res.value[1].memory_candidates == ()
    assert res.value[1].selection_summary is None
    schema = mock_generate.call_args.kwargs["config"].response_schema
    properties = schema.properties["posts"].items.properties
    assert properties["memory_candidates"].max_items == 8
    assert properties["memory_candidates"].items.max_length == 500
    assert properties["selection_summary"].max_length == 400
...
@pytest.mark.parametrize(
    "memory_fields",
    [
        {"memory_candidates": "not an array"},
        {"memory_candidates": [123]},
        {"memory_candidates": ["x"] * 9},
        {"memory_candidates": ["x" * 501]},
        {"selection_summary": 123},
        {"selection_summary": "x" * 401},
    ],
)
async def test_times_rejects_invalid_character_memory(
    memory_fields: dict[str, object],
) -> None:
    """Reject malformed Times memory before any posts can be delivered."""
    client, generate = _create_mock_client()
    response = MagicMock(spec=types.GenerateContentResponse)
    response.candidates = []
    response.usage_metadata = None
    response.text = json.dumps(
        {"posts": [{"character_name": "Dorothy", "content": "話題", **memory_fields}]}
    )
