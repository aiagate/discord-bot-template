...
登録済みの全キャラクターに調査・コーディング・文書作成・検証を依頼できます。各自の性格や口調を引き継ぎ、依頼に応じて必要な作業を選びます。Codex Python SDK経由で作業ごとにApp Serverを起動します（Geminiの設定は不要）。
...
CODEX_WORK_GUILD_ID=123456789012345678
CODEX_WORK_CHANNEL_IDS=234567890123456789
CODEX_WORK_USER_IDS=345678901234567890
# 作業報告用Webhook。空ならDISCORD_CHARACTER_WEBHOOK_URLS_JSONを使用。
CODEX_WORK_WEBHOOK_URLS_JSON='["https://discord.com/api/webhooks/..."]'
...
チャンネル・ユーザーはカンマ区切りで複数指定可能です（配下のスレッドも対象）。DM・Bot・Webhookからの依頼は受け付けません。

許可した各チャンネル（スレッドの場合は親チャンネル）にWebhookを1つ設定します。
...
- **作業場所**: `workspaces/<キャラクターID>/<作業ID>`。リポジトリ設定時は、その`HEAD`から`codex/<キャラクターID>-<作業ID>`ブランチのworktreeを作ります（元の未コミット変更は含みません）。未設定時は全員が独立フォルダを使い、調査もコード作成も可能です。既存の作業を再開するときは同じ場所を使います。
- **管理データ**: 作業記録は`tasks/`、Codex認証・会話は`codex/`に保存され、作業環境からは読み取れません。
...
- **実行制限**: 書き込みは作業場所内に限定し、OSの標準ツール等は読み取りのみ許可します。`.env*`の読み取りとBot認証情報の継承を防ぎ、シェルの通信は無効にします（Web調査はCodex内蔵検索を利用）。Git管理情報が読み取り専用のためコミットできず、通信が無効なためPushや依存関係のダウンロードもできません。テストに必要な依存が不足する場合は`!work stop`で中止し、生成された作業場所にホスト側で実行環境を用意してから再開してください。
