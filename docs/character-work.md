...
完了・失敗報告は担当者の名前とアイコンで送信します。名前・アイコンは既存の
キャラクター定義から取得します。Codexの中間進捗はWebhookへそのまま流さず、完了時は
Geminiが成果物と検証記録を確認してからキャラクターの報告を生成します。Webhook設定だけでも
利用でき、Geminiのキーがない場合は保存済み成果の完了報告にフォールバックします。
