...
class CharacterWorkSettings:
    """Store host-selected work scope and user allowlist."""
    ...
    def from_env(
        ...
        guild_value = environment.get("CODEX_WORK_GUILD_ID", "").strip()
        if not guild_value:
            guild_value = environment.get("DISCORD_CHARACTER_GUILD_ID", "").strip()
        guilds = _ids(guild_value)
        if len(guilds) != 1:
        ...
        channel_value = environment.get("CODEX_WORK_CHANNEL_IDS", "").strip()
        return cls(
            root=root,
            guild_id=next(iter(guilds)),
            channel_ids=_ids(channel_value) if channel_value else frozenset(),
