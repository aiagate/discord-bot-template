...
def split_discord_content(
    ...
    """Preserve Unicode and user mentions while reopening split fenced code blocks."""
    if limit < 64:
    ...
    while remaining:
        ...
        for separator in ("\n\n", "\n", "。", ". ", " "):
            ...
            if position > 0:
                ...
                break
        mention_start = remaining.rfind("<@", 0, cut)
        if mention_start >= 0:
            mention = _USER_MENTION.match(remaining, mention_start)
            if mention is not None and mention.end() > cut:
                cut = mention_start
        body = remaining[:cut]
