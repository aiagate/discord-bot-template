...
class DiscordWebhookTimesPublisher(ITimesPublisher):
    ...
    async def _remember_post(
        self, plan: TimesEpisodePlan, character_id: str, observed_at: datetime
    ) -> None:
        """Save a confirmed post's notes before advancing delivery progress."""
        post = plan.posts[plan.next_post_index]
        source_id = f"times:{plan.source_message_id}:{plan.next_post_index:02d}"
        if post.memory_candidates:
            memories = [
                CharacterMemory(
                ...
                for sequence, content in enumerate(post.memory_candidates)
            ]
            saved = await self._memory_store.save(memories)
            if is_err(saved):
                raise RuntimeError("Times character memories could not be saved.")
        if post.selection_summary is not None:
            summary = CharacterMemorySummary(
                character_id=character_id,
                source_message_id=source_id,
                content=post.selection_summary,
                observed_at=observed_at,
            )
            saved_summary = await self._memory_store.save_selection_summary(summary)
            if is_err(saved_summary):
                raise RuntimeError("Times character memory summary could not be saved.")
        ...
        while not current_plan.complete:
            ...
            if next_chunk_index == len(chunks):
                await self._remember_post(
                    current_plan, character.character_id, sent.created_at
                )
                next_index += 1
                next_chunk_index = 0
