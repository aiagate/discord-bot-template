...
class GenericRepository[T, K](IRepositoryWithId[T, K]):
    ...
    async def update(self, entity: T) -> Result[T, RepositoryError]:
        """Update a versioned entity, rejecting append-only and unversioned entities.

        Returns VERSION_CONFLICT for stale versions and NOT_FOUND for missing rows.
        Successful updates increment the version and refresh auditable timestamps.
        """
        ...
        if not isinstance(entity, IVersionable):
            return Err(
                RepositoryError(
                    type=RepositoryErrorType.UNEXPECTED,
                    message=(
                        f"{self._entity_type.__name__} requires a version for updates"
                    ),
                )
            )
