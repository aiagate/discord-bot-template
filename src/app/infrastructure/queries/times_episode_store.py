...
class SQLAlchemyTimesEpisodeStore(ITimesEpisodeStore):
        ...
        try:
            async with self._session_factory() as session, session.begin():
                ...
                if row is not None:
                    ...
                    if previous.next_post_index > plan.next_post_index:
    ...
    async def pending(
        ...
        """Return unfinished episode plans in the order they were created."""
        try:
            async with self._session_factory() as session:
    ...
    async def get_recent_completed(
        ...
        *,
        ...
        try:
            async with self._session_factory() as session:
                table = cast(Any, TimesEpisodeORM).__table__
                query = select(TimesEpisodeORM).where(table.c.status == "COMPLETED")
