...
class GeminiCharacterResponseGenerator(ICharacterResponseGenerator):
    ...
    async def generate(
        ...
        *,
        ...
        response_schema = types.Schema(
            ...
            properties={
                ...
                "memory_candidates": types.Schema(
                    ...
                    items=types.Schema(
                        type=types.Type.STRING,
                        max_length=MAX_CHARACTER_MEMORY_LENGTH,
                    ),
                    max_items=MAX_MEMORY_CANDIDATES,
                ),
                "selection_summary": types.Schema(
                    type=types.Type.STRING,
                    max_length=MAX_CHARACTER_MEMORY_SUMMARY_LENGTH,
                ),
            },
            required=["content"],
        )
        ...
        raw_candidates = result.value.get("memory_candidates", [])
        ...
        if not isinstance(raw_candidates, list) or not all(
            ...
            return Err(
                CharacterGenerationError(
                    type=CharacterGenerationErrorType.INVALID_RESPONSE,
                    message="Gemini response has invalid memory candidates.",
                )
            )
        candidates = tuple(
        ...
        if len(candidates) > MAX_MEMORY_CANDIDATES or any(
            ...
            return Err(
                CharacterGenerationError(
                    type=CharacterGenerationErrorType.INVALID_RESPONSE,
                    message=(
                        "Gemini response has too many or too-long memory candidates."
                    ),
                )
            )

        raw_summary = result.value.get("selection_summary")
        if raw_summary is None:
            ...
            return Err(
                CharacterGenerationError(
                    type=CharacterGenerationErrorType.INVALID_RESPONSE,
                    message="Gemini response has an invalid selection summary.",
                )
            )
        else:
            selection_summary = raw_summary.strip() or None
            if (
                selection_summary is not None
                and len(selection_summary) > MAX_CHARACTER_MEMORY_SUMMARY_LENGTH
            ):
                return Err(
                    CharacterGenerationError(
                        type=CharacterGenerationErrorType.INVALID_RESPONSE,
                        message="Gemini response has a too-long selection summary.",
                    )
                )
        return Ok(
            GeneratedCharacterResponse(
    ...
    async def generate_times_episode(
        ...
        *,
        ...
        for item in raw_posts:
            ...
            if cleaned_content:
                posts.append(
                    TimesPost(character_name=canonical_name, content=cleaned_content)
                )
